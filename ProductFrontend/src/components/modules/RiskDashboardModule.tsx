...
export function RiskDashboardModule({ onModuleChange }: RiskDashboardModuleProps = {}) {
  ...
  const [internalDrivers, setInternalDrivers] = useState<any[]>([]);
    ...
    const fetchAllData = async () => {
      ...
      try {
        const [
          ...
          fetch('http://127.0.0.1:8000/api/risk/summary').then(r => r.json()),
          fetch('http://127.0.0.1:8000/api/risk/posture').then(r => r.json()),
          fetch('http://127.0.0.1:8000/api/risk/category-breakdown').then(r => r.json()),
          fetch('http://127.0.0.1:8000/api/risk/business-units').then(r => r.json()),
          fetch('http://127.0.0.1:8000/api/risk/internal-drivers').then(r => r.json()),
          fetch('http://127.0.0.1:8000/api/risk/external-drivers').then(r => r.json()),
          fetch('http://127.0.0.1:8000/api/risk/recommendations').then(r => r.json())
        ]);

        setSummary(summaryRes || {});
        setPosture(postureRes.categories || []);
        setCategoryBreakdown(categoryRes.breakdown || []);
        setBusinessUnits(buRes.units || []);
        setInternalDrivers(internalRes.drivers || []);
        setExternalDrivers(externalRes.drivers || []);
  ...
  const fallbackPosture = [
    {
      ...
      peerGroup: 7.1,
      momChange: -0.3,
      keyFactors: 'Unpatched Apache servers, legacy OS versions'
    },
    {
      factor: 'Compliance',
      current: 6.8,
      ...
      peerGroup: 6.2,
      momChange: +0.2,
      keyFactors: 'GDPR data inventory gaps, SOC 2 audit findings'
    },
    {
      factor: 'Threats',
      current: 7.5,
      ...
      peerGroup: 7.4,
      momChange: +0.4,
      keyFactors: 'APT29 activity, ransomware campaigns'
    },
    {
      factor: 'Assets',
      current: 5.2,
      ...
      peerGroup: 5.8,
      momChange: -0.1,
      keyFactors: 'Shadow IT discovery, IoT device sprawl'
    },
    {
      ...
      peerGroup: 6.7,
      momChange: -0.5,
      keyFactors: 'MFA coverage gaps, privileged access reviews'
    },
    {
      ...
      peerGroup: 7.0,
      momChange: -0.2,
      keyFactors: 'Exposed services, network segmentation'
    }
  ];
  ...
  const fallbackBusinessUnits = [
    ...
    {
      unit: 'Corporate Network',
      risk: 7.1,
      assets: 234,
      vulnerabilities: 45,
      severity: 'High',
      riskContribution: 22.3,
      momChange: +0.3,
      trend: 'worsening',
      assetCriticality: 78
    },
    {
      unit: 'Remote Endpoints',
      risk: 6.9,
      assets: 445,
      vulnerabilities: 67,
      severity: 'High',
  ...
  const fallbackInternalDrivers = [
    {
      rank: 1,
      category: 'Unpatched Systems',
      riskScore: 8.5,
      count: 89,
      correlation: 'INC-2024-0045',
      description: '89 production servers missing critical patches'
    },
    {
      rank: 2,
      ...
      riskScore: 7.8,
      count: 234,
      correlation: 'INC-2024-0031',
      description: '234 admin accounts without MFA'
    },
    {
      rank: 3,
      ...
      count: 67,
      correlation: null,
      description: '67 S3 buckets with public exposure'
    },
    {
      rank: 4,
      ...
      count: 45,
      correlation: null,
      description: 'Over-privileged access to customer databases'
    }
  ];

  const fallbackExternalDrivers = [
    {
      rank: 1,
      category: 'APT29 Campaign Targeting',
      riskScore: 8.8,
      threatActor: 'APT29 (Cozy Bear)',
      description: 'Active targeting of Apache servers with CVE-2024-12345',
      correlation: 'INC-2024-0045'
    },
    {
      rank: 2,
      ...
      description: '23 internet-facing services with known vulnerabilities',
      correlation: null
    },
    {
      rank: 3,
      ...
      description: 'Finance-themed phishing targeting employees',
      correlation: 'INC-2024-0039'
    },
    {
      rank: 4,
      category: 'Supply Chain Risk',
      riskScore: 6.5,
      threatActor: 'Nation-state actors',
      description: 'Third-party vendor security assessments overdue',
      correlation: null
    }
  ];
  ...
  const fallbackRecommendations = [
    {
      id: 1,
      priority: 'Critical',
      action: 'Patch CVE-2024-12345 on 23 production servers',
    ...
    },
    {
      ...
      exposuresClosed: 234,
      affected_assets: 145,
      businessUnits: ['Corporate Network', 'IT Operations'],
      businessUnitImpact: 8.5,
      roi: 'High'
    },
    {
      id: 3,
      ...
      businessUnitImpact: 7.8,
      roi: 'Medium'
    },
    {
      id: 4,
      priority: 'High',
      action: 'Update firewall rules to reduce exposed services',
      ...
      businessUnitImpact: 7.2,
      roi: 'High'
    },
    {
      id: 5,
      priority: 'Medium',
      action: 'Conduct third-party vendor security assessments',
      description: 'Complete overdue vendor risk assessments to address supply chain vulnerabilities.',
      currentRisk: 7.2,
      projectedRisk: 6.8,
      riskReduction: 0.4,
      effort: 'Medium',
      ...
      businessUnitImpact: 6.5,
      roi: 'Low'
    }
  ];

  // Current data (real or fallback)
  const currentSummary = { ...fallbackSummary, ...summary };
  const currentPosture = posture.length > 0 ? posture : fallbackPosture;
  const currentCategory = categoryBreakdown.length > 0 ? categoryBreakdown : fallbackCategory;
  const currentBusinessUnits = businessUnits.length > 0 ? businessUnits : fallbackBusinessUnits;
  const currentInternalDrivers = internalDrivers.length > 0 ? internalDrivers : fallbackInternalDrivers;
  const currentExternalDrivers = externalDrivers.length > 0 ? externalDrivers : fallbackExternalDrivers;
  const currentRecommendations = recommendations.length > 0 ? recommendations : fallbackRecommendations;
  ...
  const overallRisk = {
    ...
    monthlyTrend: currentSummary.monthlyTrend || fallbackSummary.monthlyTrend
  };

  const topRiskDrivers = currentSummary.top_drivers || fallbackSummary.top_drivers;

  const riskStoryline = currentSummary.monthly_story || fallbackSummary.monthly_story;
